...
export function GroupsProvider({
  ...
  const loadTables = useCallback(async () => {
    ...
    try {
      setRoster(await repository.listRoster())
    } catch (cause) {
  ...
  const disbandGroup = useCallback(
    async (id: string) => {
      ...
      setRoster((current) => current.filter((seat) => seat.groupId !== id))
      return true
    },
    [repository, write],
  )
  ...
  const invitePlayer = useCallback(
    async (groupId: string, address: string) => {
      ...
      if (!ok) return false
      setRoster((current) => [...current.filter((entry) => entry.id !== seat.id), seat])
      return true
    },
    [displayName, email, mastered, repository, roster, uid, write],
  )
  ...
  const revokeSeat = useCallback(
    async (id: string) => {
      ...
      if (!ok) return false
      setRoster((current) => current.filter((seat) => seat.id !== id))
      return true
    },
    [repository, write],
  )
